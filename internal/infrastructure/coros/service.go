...
func (s *corosService) ActivityList(_ context.Context, size, pageNumber, modeList int) (map[string]interface{}, error) {
	token, loginErr := s.Login(context.Background())
	if loginErr != nil {
		return nil, loginErr
	}
...
func (s *corosService) Login(_ context.Context) (string, error) {
	// 检查是否有未过期的 token
	s.tokenMutex.Lock()
...
func (s *corosService) SportsSummary(_ context.Context, labelId, sportType string) (*domain.SportsSummary, error) {
	// 1. 首先获取access token
